...
parsePhone <- function(text) {
	...
	if(substring(text, dl+1, dl+1) == "0"){
	  z <- 1
	}
	  
	ptel <- substring(text, regexpr(fraza, text)+dl+z, regexpr(fraza, text)+dl+z+9)
...
parseMail <- function(text){
	...
	if(length(grep(pattern = "e-mail", text)) > 0){
		if(regexpr('.pl', text) > 0 && regexpr('.pl', text) > regexpr('e-mail', text)){
			adresEmail <- substring(text, regexpr('e-mail', text)+6, regexpr('.pl', text)+2)	
		}
	} else {
		if(length(grep(pattern = "email", text)) > 0){
			if(regexpr('.pl', text) > 0 && regexpr('.pl', text) > regexpr('email', text)){
				adresEmail <- substring(text, regexpr('email', text)+5, regexpr('.pl', text)+2)	
			}
		} 
...
for(i in 1 : length(danecsv[, "domena"]))
{
	...
	if(checkURL(url)){
		...
		if(length(grep(pattern = "T|tel(\\.|:|[:space:])", dane)) > 0){
			m <- min(grep(pattern = "T|tel(\\.|:|[:space:])", dane))
			if(m > 0) { 
			  text[i] <- try(clearText(dane[m]), silent=T)
			  tel[i] <- parsePhone(text[i])
			  if(length(grep(pattern = "[0-9]{9}", tel[i])) == 0 || nchar(tel[i]) > 9){
			  		m <- max(grep(pattern = "T|tel(\\.|:|[:space:])", dane))
			  		text[i] <- try(clearText(dane[m]), silent=T)
			  		tel[i] <- parsePhone(text[i])
			   }
