  ...
  getSocialMediaLinks <- function(v){
    ...
    vSocial <- c("facebook.com/","instagram.com/","linkedin.com/","allegro.pl/",".otomoto.pl","plus.google.com/","youtube.com/")
    vFound <- c("","","","","","","")
    for(i in 1:length(vSocial)){
      if(is.na(v[min(grep(pattern = vSocial[i], v))]) == TRUE){
        vFound[i] <- ""
      } else {
        vFound[i] <- v[min(grep(pattern = vSocial[i], v))]
        vFound[i] <- gsub("\r", "", vFound[i], fixed = TRUE)
        vFound[i] <- gsub("\n", "", vFound[i], fixed = TRUE)        
      }
    }
