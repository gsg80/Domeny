  ...
  webCrawling <- function(f, ver, ifirst = 1, ilast = 10){
  #ver <- "2.05"
  ...
  #		}
  	...
  	# 							)		
  	# 							
  	# 	write.csv(tabelaOut, "domeny_out.csv", fileEncoding = "CP1250")
  	# 	
  	# 	lnks <- unique (grep(links2match, allLinks, value=TRUE))
  	# 	linksOut <- data.frame(url2 <- lnks)
  	# 	write.csv(linksOut, "linki.csv", fileEncoding = "CP1250")
  	# }
  ...
  webCrawling("domeny.csv", "2.05", 1, 10)