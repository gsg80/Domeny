...
for(i in 1 : length(danecsv[, "domena"]))
{
	...
	if(checkURL(url)){
		...
		kontakt <-links[grep(pattern = "(K|k)ontakt", links)]
	  ...
	  Encoding(daneTitle) <- "iso-8859-2"
		...
		if(length(kontakt) > 0){
			if(checkURL(links[min(grep(pattern = "(K|k)ontakt", links))])){
				urlKontakt[i] <- links[min(grep(pattern = "(K|k)ontakt", links))]
				#print(paste(url, "1", sep=" - "))	
				}
			else{
				urlKontakt[i] <- paste(url,links[min(grep(pattern = "(K|k)ontakt", links))],sep="")
		    ...
		    links_v2 <- ""
		    dl <- 0
		    try(dl <- length(grep(pattern="(K|k)ontakt", xpathSApply(html, '//a', xmlValue))))
		      if(dl > 0){
		        links_v2 <- xpathSApply(html, '//a', xmlGetAttr, 'href')
		        urlKontakt[i] <- links_v2[min(grep(pattern="(K|k)ontakt", xpathSApply(html, '//a', xmlValue)))]
		      } else {
				    urlKontakt[i] <- url
		      }
